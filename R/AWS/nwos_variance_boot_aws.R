                         ...
                         stratum.area, units="ownerships", stat="total", R=1000)
{
  ...
  if(stat=="mean")
  {
    ...
    nwosMeanBoot <- function(df, indices)
    {
      d <- df[indices,]
      rr <- nwosResponseRates(d$point.count, d$response)
      w <- nwosWeights(d$point.count, d$area, d$domain, stratum.area, rr)
    ...
    b <- boot(data=df, statistic=nwosMeanBoot,  R=R)
  } 
  ...
  if(stat=="proportion")
  {
    # x.r <- nwosProportion(weight=w, point.count=df.r$point.count,
                          # domain=df.r$domain, y=df.r$y, df.r$area, units=units)
    nwosProportionBoot <- function(df, indices)
    {
      d <- df[indices,]
      rr <- nwosResponseRates(d$point.count, d$response)
      w <- nwosWeights(d$point.count, d$area, d$domain, stratum.area, rr)
    ...
    b <- boot(data=df, statistic=nwosProportionBoot,  R=R)
  }
