...
nwos_stratum_area_replicates <- function(index, data, stratum.name, state.area)
{
  data <- data.frame(stratum = unlist(data[index, stratum.name]), point.count = 1) # Create data frame
  nwos_stratum_area(stratum = data$stratum, point.count = data$point.count, state.area = state.area) # Generate estimats
}
