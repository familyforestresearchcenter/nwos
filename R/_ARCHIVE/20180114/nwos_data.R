...
nwosData <- function(sample.data, long.form=F)
{
  ...
  if(long.form)
  {
    ...
    # Recode Domain
    nwos.data$domain <- as.character(nwos.data$domain)
    domain.table <- data.frame(OWNCD=c(1,2,3,4,5,6,7,0,9),
                               OWNCD.NAME=c("FamilyForest", "CorporateForest", "OtherPrivateForest",
                                            "TribalForest",
                                            "FederalForest", "StateForest", "LocalForest",
                                            "UnknownForest", "Nonforest"))
    nwos.data$domain<- as.character(domain.table$OWNCD.NAME[match(nwos.data$domain,domain.table$OWNCD)])
    nwos.data$domain <- factor(nwos.data$domain, levels=c("FamilyForest", "CorporateForest", "OtherPrivateForest",
                                                          "TribalForest",
                                                          "FederalForest", "StateForest", "LocalForest",
                                                          "UnknownForest", "Nonforest"))
    ...
    nwos.data$response <- factor(ifelse(nwos.data$domain%in%private.forest,
                                        nwos.data$response, NA))
    nwos.data$response <- factor(nwos.data$response)

    nwos.data$area <- ifelse(nwos.data$domain%in%private.forest & nwos.data$response==1,
                             nwos.data$area, NA)

    # Auxilarry Variables
    nwos.data$y_1 <- factor(ifelse(nwos.data$domain%in%private.forest & nwos.data$response==1,
                                   nwos.data$y_1, NA))
    nwos.data$y_2 <- factor(ifelse(nwos.data$domain%in%private.forest & nwos.data$response==1,
                                   nwos.data$y_2, NA))
    nwos.data$y_3 <- round(ifelse(nwos.data$domain=="FamilyForest" & nwos.data$response==1,
                                  nwos.data$y_3, NA),0)
  }
  else
  {
    ...
    domain.table <- data.frame(OWNCD=c(1,2,3,4,5,6,7,0,9),
                               OWNCD.NAME=c("FamilyForest", "CorporateForest", "OtherPrivateForest",
                                            ...
                                            "FederalForest", "StateForest", "LocalForest",
                                            "UnknownForest", "Nonforest"))
    sample.data$d <- as.character(domain.table$OWNCD.NAME[match(sample.data$d,domain.table$OWNCD)])
    sample.data$d <- factor(sample.data$d, levels=c("FamilyForest", "CorporateForest", "OtherPrivateForest",
                                                    "TribalForest",
                                                    "FederalForest", "StateForest", "LocalForest",
                                                    "UnknownForest", "Nonforest"))
    nwos.data$domain <- as.factor(sample.data$d[match(nwos.data$id, sample.data$i)])
    ...
    # Area
    # nwos.data$area <- round(ifelse(nwos.data$domain%in%private.forest & nwos.data$response==1,
