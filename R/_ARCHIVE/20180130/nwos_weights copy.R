...
nwosWeights <- function(point.count, area, owner.class, stratum.area, response.rate, response.rate.owner.class)
{
  n <- sum(point.count) # Sample size

  w <- stratum.area / (n * area) # Base weight

  rr <- response.rate[match(owner.class, response.rate.owner.class)] # Response for each observation

  w <- w * (1 / rr) # Adjust for owner class response rates
