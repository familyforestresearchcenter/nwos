...
The wide format contains one record for every valid, returned survey and one column for every question and every piece of metadata. The NWOS is conceived as a ‘two-phase’ sample, however, with points as the primary sampling unit and ownerships as a secondary sampling unit. For some purposes, such as analysis using the ‘survey’ package (Lumley 2019^[T. Lumley (2019) "survey: analysis of complex survey samples". R package version 3.35-1]), the entire sample (including all sample points) is needed. To that end, the nwos_full() function is able to generate a wide dataframe with one record for every sample point and, where a sample point is associated with a valid survey, one column for every question and every piece of metadata. The full table (and corresponding metadata table) can be generated by:

```{r full}
qfull <- nwos_full(quest, imputations='1')
qfmd <- nwos_full_metadata(quest)
```

For still other purposes, including certain plotting and statistical functions, a long table may be preferred. This is a table where each question – not each questionnaire – is represented is by an individual row. A long table (and corresponding metadata table) can be generated by:
...
Finally, many users will want to be able to export quest data in other formats to use with software other than R. Fortunately, prepared response data (both long and wide formats), ‘completed’ plot tables, and individual slots of ‘nwos.object’s and ‘nwos.plot.object’s all take the form of regular R dataframes and can be exported in a number of formats using standard R functions. Additionally, the nwos_export() function can be used to export response data as text in any of the ‘wide’,’long’, or ‘full’ formats:
