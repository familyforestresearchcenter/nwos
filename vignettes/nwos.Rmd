...
WI_FFO_AREA_REP <- sapply(WI_REPLICATES, nwos_stratum_area_apply, stratum = wi$FFO, point.count = wi$POINT_COUNT, state.area = 33898733)
```
...
```{r proportions}
WI_FFO_OWN_Y1_PROP <- nwos_proportion(weight = wi$WEIGHT, domain = wi$Y_1)
...
WI_FFO_AC_Y1_PROP <- nwos_proportion(weight = wi$WEIGHT, area = wi$AC_WOOD, domain = wi$Y_1)
...
WI_FFO_OWN_AC_QUANTILE_REP <- sapply(1:2, # length(WI_REPLICATES),
                                     nwos_quantile_replicates, index.rep = WI_REPLICATES, data = wi,
